...
def simulate_from_payload(payload: dict[str, object]) -> dict[str, object]:
    ...
    return {
        "summary": {
            ...
            "bed_water_g": float(summary["bed_water_g"]),
        ...
        output.append(
            {
                ...
                # Public "retained_water_g" matches the summary semantics:
                # total water held in the coffee bed = mobile pore + immobile retained.
                "retained_water_g": float(row["bed_water_g"]),
                "bed_water_g": float(row["bed_water_g"]),
                "mobile_bed_water_g": float(row["pore_water_g"]),
                "immobile_retained_water_g": float(row["retained_water_g"]),
